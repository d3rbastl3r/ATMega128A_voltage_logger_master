PROJECT=atmega128a_voltage_logger_master
PARTNO=m128
MCU=atmega128
BITCLOCK=1# Adjust the Frequency of the Programmer. Use 1 on >=4MHz, 32 >=128KHz
PORT=usb
...
fuse_w:
	avrdude $(AVRDUDE_FLAGS) -U hfuse:w:$(PROJECT).hfuse:r -U lfuse:w:$(PROJECT).lfuse:r -U efuse:w:$(PROJECT).efuse:r